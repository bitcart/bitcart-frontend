...
test.describe("Landing: Coins page (no JavaScript)", () => {
  ...
  test("coin names are server-rendered without JavaScript", async ({ page }) => {
    ...
    await expect(page.getByLabel("Statistics").getByText("Supported Blockchains")).toBeAttached()
    ...
    await expect(
      page.getByRole("button", { name: /Bitcoin BTC/, includeHidden: true }),
    ).toBeAttached()

    await expect(
      page.getByRole("button", { name: /Ethereum ETH/, includeHidden: true }),
    ).toBeAttached()
  })
})
