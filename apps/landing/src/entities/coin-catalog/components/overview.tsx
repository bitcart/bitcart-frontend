  ...
  return (
    <section aria-labelledby="coin-catalog-heading" className="py-12 bg-secondary sm:py-20">
      <div className="px-4 max-w-7xl lg:px-8 md:px-6 mx-auto">
        <h2 id="coin-catalog-heading" className="sr-only">{t`Supported Blockchains`}</h2>

        <span className="sr-only" aria-live="polite" aria-atomic="true">
          {searchCounts.blockchains === 0
            ? t`No blockchains found`
            : t`${searchCounts.blockchains} blockchains found`}
        </span>

        {searchCounts.blockchains === 0 ? (
          <div className="sm:py-12 sm:h-77 text-center">
                      ...
                      <div className="min-w-0 space-x-3 sm:space-x-4 flex flex-1 items-center">
                        ...
                        >
                          <CoinCatalogEntryIcon
                            size={32}
                            symbol={blockchain.intrinsicTokenSymbol}
                          />
                        </div>
                  ...
                  {fungibleTokens.entries.length > 0 && (
                    <AccordionContent className="p-4 sm:p-6 border-t">
                      ...
                      <p className="text-xs text-muted-foreground mb-3 sm:text-sm sm:mb-4">
                        <Trans>
                          These are just examples. Any ERC-20/BEP-20/TRC-20/CashToken compatible
                          token can be used by entering its contract address.
                        </Trans>
                      </p>
                        ...
                        {fungibleTokens.entries.map((tokenSymbol) => {
                          return (
                            <div
                              ...
                              className={cn(
                                `
                                  rounded-md p-2 group gap-1.5 flex flex-col items-center
