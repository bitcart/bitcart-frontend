...
export const createOnPageTransitionEnd = (): OnPageTransitionEndSync => {
  return function onPageTransitionEnd(pageContext) {
    const hash = pageContext.urlParsed.hash

    //! Defer to the next macrotask. Firefox cancels in-flight smooth
    //! scrolls if the document's scroll position changes around the same
    //! tick (e.g. the browser auto-clamping `scrollY` when the new SPA
    //! page is shorter than the previous one), so the animation collapses
    //! into an instant jump. setTimeout(0) lets that clamp happen first
